...
def manager_repr(value):
    ...
    for manager_name, manager_instance in names_and_managers:
        if manager_instance == value:
            return "{}.{}.all()".format(model._meta.object_name, manager_name)
    return repr(value)
...
def field_repr(field, force_many=False):
    ...
    kwarg_string = ", ".join(
        ["{}={}".format(key, smart_repr(val)) for key, val in sorted(kwargs.items())]
    )
    ...
    return "{}({}{})".format(class_name, arg_string, kwarg_string)
