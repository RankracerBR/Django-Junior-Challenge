...
class Config:
    ...
    def _load(cls, configfiles, verbose=False):
        ...
        if os.path.exists(".node-version"):
            with open(".node-version") as v_file:
...
def parse_args(check=True):
    ...
    if args.config_file is None:
        ...
        if not os.path.exists(args.config_file):
            parser.error("Config file '{}' doesn't exist!".format(args.config_file))
...
def callit(cmd, show_stdout=True, in_shell=False, cwd=None, extra_env=None):
    ...
    try:
        ...
        e = sys.exc_info()[1]
        logger.error("Error {} while executing command {}".format(e, cmd_desc))
        raise
...
def get_root_url(version_str):
    if parse_version(version_str) > (0, 5):
        return "{}/v{}/".format(src_base_url, version_str)
    else:
        return src_base_url
...
def get_node_bin_url(version):
    ...
    filename = "node-v{}{}".format(version, postfix)
...
def urlopen(url):
    ...
    headers = {"User-Agent": "nodeenv/{} ({})".format(nodeenv_version, home_url)}
...
def build_node_from_src(env_dir, src_dir, node_src_dir, args):
    ...
    make_opts = [
        f"--{name}={value}" if len(value) > 0 else f"--{name}"
...
def install_node_wrapped(env_dir, src_dir, args):
    ...
    logger.info(
        " * Install {} node ({}) ".format(src_type, args.node),
...
def install_npm_win(env_dir, src_dir, args):
    ...
    if is_CYGWIN:
        ...
        npm_bin_url = f"{npm_gh_url}/{args.npm}/bin/npm"
