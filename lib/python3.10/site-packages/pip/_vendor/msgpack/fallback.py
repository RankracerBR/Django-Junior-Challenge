    ...
    def _read_header(self):
        ...
        if b & 0b10000000 == 0:
            ...
            if n > self._max_str_len:
                raise ValueError(
                    "{} exceeds max_str_len({})".format(n, self._max_str_len)
                )
            obj = self._read(n)
            ...
            if n > self._max_map_len:
                raise ValueError(
                    "{} exceeds max_map_len({})".format(n, self._max_map_len)
                )
        elif b == 0xC0:
            obj = None
        elif b == 0xC2:
            ...
            if n > self._max_bin_len:
                raise ValueError(
                    "{} exceeds max_bin_len({})".format(n, self._max_bin_len)
                )
            obj = self._read(n)
        elif 0xC7 <= b <= 0xC9:
            size, fmt, typ = _MSGPACK_HEADERS[b]
            ...
            if L > self._max_ext_len:
                raise ValueError(
                    "{} exceeds max_ext_len({})".format(L, self._max_ext_len)
                )
            obj = self._read(L)
        elif 0xCA <= b <= 0xD3:
