    ...
    for k, v in exports.items():
        ...
        for entry in v.values():
            if entry.suffix is None:
                ...
                s = "{}:{}".format(entry.prefix, entry.suffix)
            if entry.flags:
...
def get_project_data(name):
    url = "{}/{}/project.json".format(name[0].upper(), name)
...
def get_package_data(name, version):
    url = "{}/{}/package-{}.json".format(name[0].upper(), name, version)
    url = urljoin(_external_data_base_url, url)
...
class Sequencer:
    ...
    def remove(self, pred, succ):
        ...
        try:
            ...
            raise ValueError("{!r} not a successor of {!r}".format(succ, pred))
    ...
    def dot(self):
        ...
        for succ in self._preds:
            preds = self._preds[succ]
            for pred in preds:
                result.append("  {} -> {};".format(pred, succ))
    ...
    def ETA(self):
        if self.done:
            ...
            if self.max is None:
                ...
                t /= self.cur - self.min
                t = (t - 1) * self.elapsed
        return "{}: {}".format(prefix, self.format_duration(t))
